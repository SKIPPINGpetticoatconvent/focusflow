...
<aside class="w-64 bg-base-100 h-screen flex flex-col border-r border-base-200 shadow-xl">
	...
	<div class="p-6">
		<a href="/dashboard" class="flex items-center gap-3 group">
			...
			<div>
				...
				<p class="text-xs text-base-content/50">Productivity Tracker</p>
			</div>
		</a>
	</div>
	...
	<nav class="flex-1 px-4 py-2 space-y-1">
		<p class="text-xs font-semibold text-base-content/40 uppercase tracking-wider mb-3 px-3">Menu</p>
		...
		{#if $auth.user}
			<div class="flex items-center gap-3 mb-4 p-3 bg-base-200/50 rounded-xl">
				...
				<div class="flex-1 min-w-0">
					<p class="text-sm font-semibold truncate">{$auth.user.full_name || 'User'}</p>
